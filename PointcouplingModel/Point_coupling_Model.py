...
class MambaBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.mamba = Mamba(d_model=channels, d_state=16, d_conv=4, expand=1)
...
class RPE(torch.nn.Module):
    ...
    def forward(self, coord):
        ...
        out = out.view(idx.shape + (-1,)).sum(3).permute(0, 3, 1, 2)
        return out
...
class GSG(nn.Module):
    ...
    def forward(self, feat):
        ...
        fused_features = torch.matmul(weighted_B + weighted_Delta, self.weight_matrix)

        updated_feat = feat + fused_features
...
class SerializedAttention(PointModule):
    ...
    def forward(self, point):
        ...
        if not self.enable_flash:
            ...
            if self.enable_rpe:
                bais = self.rpe(self.get_rel_pos(point, order))
                attn = attn + self.rpe(self.get_rel_pos(point, order))
            if self.upcast_softmax:
                attn = attn.float()
            attn = self.softmax(attn)
            attn = self.attn_drop(attn).to(qkv.dtype)
            feat1 = (attn @ v).transpose(1, 2).reshape(-1, C)
        else:

            feat1 = flash_attn.flash_attn_varlen_qkvpacked_func(
            ...
            feat1 = feat1.to(qkv.dtype)

        feat = point.feat
        feat0_ordered = feat[point.serialized_order[self.order_index]]
        feat1_ordered = self.GSG(feat0_ordered)
...
class Block(PointModule):
    ...
    def forward(self, point: Point):
        ...
        feat_ordered = feat[point.serialized_order[self.order_index]]
        feat = self.drop_path(self.mamba(feat_ordered))
        feat = feat[point.serialized_inverse[self.order_index]]
        point.feat = feat
        point.feat = shortcut + point.feat
