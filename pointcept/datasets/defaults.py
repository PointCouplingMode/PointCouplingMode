...
class DefaultDataset(Dataset):
    ...
    def get_data(self, idx):
        ...
        print(f"[DEBUG] 当前数据路径: {data_path}")  # 检查路径是否正确
        print(f"[DEBUG] 路径下文件: {os.listdir(data_path)}")  # 确认包含 coord.npy
        name = self.get_data_name(idx)
        coord_path = os.path.join(data_path, "coord.npy")
        assert os.path.exists(coord_path), f"关键文件 coord.npy 缺失于路径: {data_path}"
        if self.cache:
